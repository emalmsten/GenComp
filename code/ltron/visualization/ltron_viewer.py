...
playback -----------------------------------------------------------------------
0 :                     show the original file
1 :                     show the file 10 files back
2 :                     show the previous file
3 :                     show the next file
4 :                     show the file 10 files forward
C :                     save current scene
Q :                     play all files (preview of gif)
G :                     save gif
5 :                     decrease gif fps by 10
6 :                     decrease gif fps by 1
7 :                     increase gif fps by 1
8 :                     increase gif fps by 10
------------------------------------------------------------------------
...
def start_viewer(
    ...
    def rotate_instance(instance_id, snap_id, axis, angle):
        transform_about_snap(instance_id, snap_id, get_rotation_matrix(axis, angle))

    def random_rotation(all_bricks, axis, discrete=False):
        random_brick = random.choice(list(all_bricks.instances.values()))
        brick_id = random_brick.instance_id

        random_snap = random.choice(random_brick.snaps)
        snap_id = random_snap.snap_style.snap_id

        random_angle = random.choice([0, 90, 180, 270]) if discrete else random.uniform(0, 360)
        print('Rotating %s by %s degrees' % (random_brick, random_angle))
        rotate_instance(brick_id, snap_id, axis, random_angle)

    def get_translation_matrix(axis, distance):
        if axis == 'x':
            return numpy.array([
                [1, 0, 0, distance],
                [0, 1, 0, 0],
    ...
    def translate_instance(instance_id, axis, distance):
        transform = scene.instances[instance_id].transform
        transform = transform @ get_translation_matrix(axis, distance)
        scene.move_instance(instance_id, transform)

    def random_translation(all_bricks, axis, discrete=False):
        discrete_dict = {
            'x': 20,
            'y': 8,
            'z': 20
        }
        discrete_choices = numpy.array([-2, -1, 0, 1, 2]) * discrete_dict[axis]
        random_brick = random.choice(list(all_bricks.instances.values()))
        brick_id = random_brick.instance_id

        random_distance = random.choice(discrete_choices) if discrete else random.uniform(-40, 40)
        print('Translating %s by %s units' % (random_brick, random_distance))

        translate_instance(brick_id, axis, random_distance)


    # Under here until key_press are the functions from Emil
    def show_file(fp, verbose=True):
        if verbose:
            print(f"Playing file: {fp}")

        scene.instances.clear()
        scene.import_ldraw(fp)
        render()

    def play_scene(gif_fps):
        for fp in play_files:
            show_file(fp, verbose=False)
            time.sleep(1 / gif_fps)

    def get_new_filename(fp, prefix, extension):
        os.makedirs(fp, exist_ok=True)
        i = 0
        while True:
            i += 1
            filename = f'{fp}/{prefix}_{i}.{extension}'
            if not os.path.exists(filename):
                break
        return filename

    def save_capture():
        fp = f'{save_dir}/captured_mpds'
        filename = get_new_filename(fp, 'export', 'mpd')
        scene.export_ldraw(filename)
        print('Saved capture to:', filename)

    def save_gif(gif_fps):
        import imageio
        import numpy as np
        images = []
        for file_path in play_files:
            show_file(file_path, verbose=False)
            pixels = window.read_pixels()
            flipped_pixels = np.flipud(pixels)  # Flip the image vertically
            images.append(flipped_pixels)

        print('Saving gif..')
        save_path = f'{save_dir}/animation'
        save_name = get_new_filename(save_path, 'animation', 'gif')
        duration = 1000 / gif_fps
        imageio.mimsave(save_name, images, duration=duration)  # Adjust duration for speed
        print('Saved gif to:', save_name)


    def key_press(key, x, y):
        # From here to the comment below is code from Emil
        ...
        if key == b'0':
            ...
            show_file(file_path)
        elif key == b'1' or key == b'2' or key == b'3' or key == b'4':
            inc_dict = {b'1': -10, b'2': -1, b'3': 1, b'4': 10}
            curr_file = (curr_file + inc_dict[key]) % num_files
            print("curr_file: ", curr_file)
            show_file(play_files[curr_file])

        elif key == b'5' or key == b'6' or key == b'7' or key == b'8':
            inc_dict = {b'5': -10, b'6': -1, b'7': 1, b'8': 10}
            gif_fps += inc_dict[key]
            print(f"New gif fps: {gif_fps}")
        ...
        # This is the comment below, under here is original LTRON code
        ...
        #        state['render_mode'] = 'color'
            ...
            if instance_id is None:
                ...
                # instance = scene.get_instance(instance_id)
                scene.remove_instance(str(instance_id))
                print('Removing Brick Instance %i' % instance_id)
            ...
            # get all instances
            instance_ids = list(scene.instances.keys())

            connected_snaps_per_instance = [scene.get_instance_snap_connections(str(instance_id)) for instance_id in
                                            instance_ids]
            print('Emils own print all connections:')
            for i, connected_snaps in enumerate(connected_snaps_per_instance):
                print('Instance: %i' % instance_ids[i])
                for snap_a, snap_b in connected_snaps:
                    print(snap_a, ':', snap_b)

                connected_instances = set(
                    int(snap_b.brick_instance) for snap_a, snap_b in connected_snaps
                )
    ...
    def get_snap_under_mouse(x, y, polarity=None):
        ...
        instance_id_lookup = {str(snap): int(snap.brick_instance) for snap in snaps}
        renderer.set_instance_masks_to_instance_indices(instance_id_lookup)
        part_mask_frame.enable()
        ...
        #        name : int(snap)
        #        for name, (inst, snap) in zip(snap_instance_names, snaps)}
        snap_id_lookup = {str(snap): int(snap.snap_style) for snap in snaps}
