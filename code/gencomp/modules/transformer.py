...
from gencomp.modules.util import timestep_embedding
...
class TransformerModel(nn.Module):
    ...
    def forward(self, x, timesteps, **kwargs):
        ...
        if self.condition_channels > 0:
            cond = None
            keys = ['poly', 'corner_indices', 'room_indices']
            if self.config.general.dataset == 'ltron':
                keys = ['poly']
            for key in keys:
